...
class UserButton extends React.Component {
  ...
  readableDate(){
    ...
    if (Date.now() - Date.parse(this.props.lastMessage.posted) < 86400000){
      return msgDate.getHours() + "h" + msgDate.getMinutes();
    }
  ...
  render() {
    ...
    if (this.props.lastMessage){
      if (this.props.lastMessage.owner === this.props.user.uid){
        lastMessage = (this.props.lastMessage.viewed) ? <div className="last-message"><em>{this.readableDate()}</em><i>{this.props.lastMessage.text}</i></div> : <div className="last-message"><em>{this.readableDate()}</em><b>{this.props.lastMessage.text}</b></div>;
      }
      else {
        lastMessage = <div className="last-message"><em><span className="glyphicon glyphicon-share-alt"></span> {this.readableDate()}</em><i> {this.props.lastMessage.text}</i></div>
      }
    }
    return (
